(function () {
    ...
    // ===== 样式处理 =====
    // 调用顺序：
    // 1) applyWechatCodeBlockFormat
    // 2) applyInlineThemeAndTypography
    // 3) applyBaseInlineStyles
    function applyBaseInlineStyles(root) {
        root.querySelectorAll("*").forEach((node) => {
            if (node.closest("[data-mdbp-code-block]")) {
                return;
            }
            app.appendInlineStyle(node, "word-break: break-word;");
        });

        root.querySelectorAll("img").forEach((img) => {
            app.appendInlineStyle(img, "max-width: 100%; height: auto;");
        });
        root.querySelectorAll("table").forEach((table) => {
            app.appendInlineStyle(table, "width: 100%; border-collapse: collapse; margin: 1em 0;");
        });
        root.querySelectorAll("th, td").forEach((cell) => {
            app.appendInlineStyle(cell, "border: 1px solid #e5e7eb; padding: 8px 10px; text-align: left;");
        });
        root.querySelectorAll("blockquote").forEach((blockquote) => {
            app.appendInlineStyle(blockquote, "margin: 0.9em 0; padding: 0.25em 0 0.25em 0.9em; border-left: 4px solid #d0d7de; color: #57606a; background: #f6f8fa;");
        });
        root.querySelectorAll("pre").forEach((pre) => {
            app.appendInlineStyle(pre, "ooverflow-x: scroll;");
        });
    }
    ...
    function applyCodeThemeStyles(root) {
        ...
        root.querySelectorAll("[data-mdbp-code-block]").forEach((node) => {
            app.applyStyleMap(node, codeTheme.blockStyle);
        });
        root.querySelectorAll("[data-mdbp-code-content]").forEach((node) => {
            app.applyStyleMap(node, codeTheme.codeStyle);
        });
        ...
        Object.entries(tokenStyles).forEach(([selector, styleMap]) => {
            ...
            root.querySelectorAll(selector).forEach((node) => {
                app.applyStyleMap(node, styleMap);
            });
        });
    }
    ...
    function applyInlineThemeAndTypography(container) {
        ...
        const rootFontFamily = app.getSelectedFontFamily();
        ...
        app.appendInlineStyle(root, `font-family: ${rootFontFamily};`);
        app.appendInlineStyle(root, `font-size: ${app.refs.fontSizeSelect.value}px;`);
        app.appendInlineStyle(root, "line-height: 1.8;");
        app.appendInlineStyle(root, "color: #2f2f2f;");
        app.appendInlineStyle(root, "word-break: break-word;");
        app.applyStyleMap(root, theme?.contentStyle);

        applyBaseInlineStyles(root);

        root.querySelectorAll("p, li, blockquote, td, th").forEach((node) => {
            app.appendInlineStyle(node, `font-family: ${rootFontFamily};`);
            app.appendInlineStyle(node, `font-size: ${app.refs.fontSizeSelect.value}px;`);
        });
        ...
        if (elementStyles && typeof elementStyles === "object") {
            ...
            if (elementStyles.a) {
                root.querySelectorAll("[data-mdbp-link-text]").forEach((node) => {
                    app.applyStyleMap(node, elementStyles.a);
                });
            }
        }

        root.querySelectorAll("pre > code").forEach((code) => {
            app.appendInlineStyle(code, `font-family: ${app.constants.FIXED_CODE_FONT};`);
        });
    ...
    function convertExternalLinksToReferences(container) {
        ...
        anchors.forEach((anchor) => {
            if (!app.shouldConvertLink(anchor)) {
                return;
            }
    ...
    function applyWechatCodeBlockFormat(rawHtml) {
        ...
        blockCodes.forEach((code) => {
            ...
            app.appendInlineStyle(pre, "margin: 1em 0; padding: 12px 14px; background: #f6f8fa; border: 1px solid #eaecef; border-radius: 6px; overflow-x: auto;");

            code.setAttribute("style", "");
            app.appendInlineStyle(code, `display: block; color: #24292e; background: transparent; font-size: 13px; line-height: 1.7; font-family: ${app.constants.FIXED_CODE_FONT};`);
        });

        const inlineCodes = container.querySelectorAll("code:not(pre > code)");
        const inlineCodeStyle = Object.assign({}, app.constants.defaultInlineCodeStyle, app.normalizeStyleObject(theme?.inlineCodeStyle));
        const inlineCodeStyleText = app.styleObjectToCss(inlineCodeStyle);
        inlineCodes.forEach((code) => {
            code.setAttribute("style", "");
            app.appendInlineStyle(code, `${inlineCodeStyleText} font-family: ${app.constants.FIXED_CODE_FONT};`);
        });
    ...
    app.render = {
        ...
        copyPreviewContent: async function copyPreviewContent() {
    ...
    function applyTheme(themeId) {
        ...
        const previewStyle = Object.assign({}, app.constants.defaultPreviewStyle, app.normalizeStyleObject(theme?.previewStyle));
        Object.assign(app.refs.previewInfo.style, previewStyle);

        app.render.applyFontSize(app.refs.fontSizeSelect.value);
    }
    ...
    function setThemeOptions(themes, nextSelectedThemeId) {
        ...
        app.render.applyTheme(app.state.selectedThemeId);
        ...
        if (app.state.latestMarkdownHtml) {
            app.render.renderMarkdownToPreview(app.state.latestMarkdownHtml);
        }
    }
